...
BASE_URL_DEFAULT = "https://lunarcrush.com/api4/public"
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            with make_client(timeout_s=settings["timeout_s"]) as client:
                r = client.get(url, headers=build_headers(token), params=params)
...
def cached_fetch(
    ...
    return fetch_memecoins(base_url, token, limit, page, sort_field, desc, filter_value)
...
st.title(APP_NAME)
st.caption("Rank memecoin candidates using social momentum signals (Galaxy Score™ / AltRank™) from LunarCrush.")
...
st.caption("Data source: LunarCrush API v4. Metrics include Galaxy Score™ and AltRank™.")
